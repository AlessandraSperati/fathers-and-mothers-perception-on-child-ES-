...
"
fitmod3Fbif <- cfa(mod3Fbif, std.lv = TRUE, orthogonal = TRUE, missing = "FIML", data = X, group = "Group")
...
fit2mod3Fbif <- cfa(mod3Fbif, std.lv=TRUE, orthogonal = TRUE, missing = "FIML", data = X, group = "Group",  
...
HSC_7 + HSC_8 + HSC_9 + HSC_10 + HSC_11 + HSC_12
"
fit3mod3Fbif <- cfa(mod3Fbif, std.lv=TRUE, orthogonal = TRUE, missing = "FIML", data = X, group = "Group",  group.equal = c("loadings", "intercepts"))

summary(fit3mod3Fbif, fit.measures=TRUE)
##please insert missing = ML for running CFAs adopting listwise deletion. 
...
##linear regression models adopting FIML as data estimation tecnique in lavaan
###################DATA ESTIMATION APPROACH - FIML

library(lavaan)
model0 <- '
  # Regression
  inter_tot ~ accept_mr + Age_child + Gen_child
'

fit0 <- sem(model0, data = M, missing = "fiml")
summary(fit0, fit.measures = TRUE, standardized = TRUE)

model1 <- '
  # Regression
  inter_tot ~ accept_mr + hsc_tot + Age_child + Gen_child
'

fit1 <- sem(model1, data = M, missing = "fiml")
summary(fit1, fit.measures = TRUE, standardized = TRUE)

M$interaction <- M$accept_mr * M$hsc_tot
model2 <- '
  # Regression
  inter_tot ~ accept_mr + hsc_tot + interaction + Age_child + Gen_child
'

fit2 <- sem(model2, data = M, missing = "fiml")
summary(fit2, fit.measures = TRUE, standardized = TRUE)
anova(fit0, fit1, fit2)


##replace exter_tot for exploring externalizing problems as outcome
##or father-related variables for running models with fathers or variables related to single informant. 



###linear regression models adopting listwise deletion
